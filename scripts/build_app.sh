...
echo "📦 Creating macOS Application Bundle structure at ${BUNDLE_DIR}..."
...
if command -v hdiutil &>/dev/null; then
    ...
    echo "💿 Generating distribution DMG at ${DMG_PATH}..."
    hdiutil create -volname "${APP_NAME}" -srcfolder "${BUNDLE_DIR}" -ov -format UDZO "${DMG_PATH}"
    echo "🎉 DMG ready for GitHub Releases & Homebrew Cask distribution!"
fi
